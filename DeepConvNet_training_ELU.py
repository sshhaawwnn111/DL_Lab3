...
import os
import argparse
from models import DeepConvNetELU
...
for epoch in range(epochs):
    ...
    if epoch%1==0:
        ...
        if test_accuracy > ELU_max_val_acc:
            ...
            if save_model:
                print("model saved")
                torch.save(model.state_dict(), 'DeepConvNetELU.bin')
...
plt.plot(epochs, ELU_train_accuracy_history, label='ELU_train', color='r')
plt.plot(epochs, ELU_test_accuracy_history, label='ELU_test', color='b')
