...
import os
import argparse
from models import EEGNetLeakyReLU
...
for epoch in range(epochs):
    ...
    if epoch%1==0:
        ...
        if test_accuracy > LeakyReLU_max_val_acc:
            ...
            if save_model:
                torch.save(model.state_dict(), 'EEGNetLeakyReLU.bin')
...
epochs = range(1, len(LeakyReLU_train_accuracy_history) + 1)
plt.plot(epochs, LeakyReLU_train_accuracy_history, label='LeakyReLU_train', color='r')
plt.plot(epochs, LeakyReLU_test_accuracy_history, label='LeakyReLU_test', color='b')
plt.title('EEGNetLeakyReLU')
plt.xlabel('Epochs')
plt.ylabel('acc')
plt.legend()
plt.show()